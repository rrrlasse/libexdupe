...
static size_t read_hashblock(hashblock_t* h, char* src) {
    ...
    for (size_t t = 0; t < SLOTS; t++) {
        ...
        if (!nulls) {
            h->hash[t] = uint32_t(str2ll(src, 4));
            src += 4;
        }
        ...
        if (nulls) {
            ...
            h->entry[t].slide = uint16_t(str2ll(src + 8, 2));
            h->entry[t].first_byte = uint8_t(str2ll(src + 10, 1));
...
const static char *dub(const char *src, uint64_t pay, size_t len, size_t block, uint64_t *payload_ref) {
    ...
    while (src + block <= last_src) {
        ...
        if (w != 0 && (e = lookup(w, block == state_c.LARGE_BLOCK))) {
            ...
            // todo, rewite, W4 gives false warning on uninitialized e_cpy
            if (e && src + block <= last_src && e_cpy.first_byte == (uint8_t)src[0] && e_cpy.offset + block < pay + (src - orig_src)) {
                char s[HASH_SIZE];
    ...
    do {
        ...
        *dst++ = 'C';
        ...
        for (size_t t = 0; t < count; t++) {
            size_t h = write_hashblock(&state_c.small_table[block], dst);
            dst += h;
            block++;
        }
